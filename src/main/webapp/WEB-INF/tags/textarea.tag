...
<%@ attribute name="rows" required="true" description="行数" %>
...
<div class="form-group">
    ...
    <textarea ${maxlen} ${pattern} name="${name}" rows="${rows}" ${readonly eq true?"readonly":"" } ${disabled eq true?"disabled":"" } ${required eq true?"required='required'":""}  id="${id}" 
      style="${style}" class="form-control ${inputClass }" title="${placeholder}" placeholder="${placeholder}">${value}</textarea>
    </div>
