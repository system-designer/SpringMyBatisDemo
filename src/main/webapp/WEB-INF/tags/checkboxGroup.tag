...
<div class="form-group">
	...
	<div id="${id}" class="${inputDivClass} ${required eq true ? 'required':'' }">
    	...
    	%>
    		<label><input type="checkbox" <%= item.getCode().equals(value)?"checked":"" %> style="margin-left:10px;margin-top: 0px;width:15px;" name="${name}" value="<%= item.getCode()%>"> <%= item.getName() %></label>
    	...
    	<c:if test="${item ne null}">
    		<c:forEach items="${item}" var="li">
    			<label><input type="checkbox" ${li[itemcode] eq value ? 'checked':''} style="margin-left:10px;margin-top: 0px;width:15px;" name="${name}" value="${li[itemcode]}"> ${li[itemtext]}</label>
    		</c:forEach>
    	</c:if>
    		...
    		<script type="text/javascript">
	    		seajs.use(['jquery','myutil.init'],function($){
	    			...
	    			function fillOption(record){
	    				...
	    				$('#${id}').append('<label><input type="checkbox" value="'+code+'"'+ selected+' style="margin-left:10px;margin-top: 0px;width:15px;" name="${name}"> '+text+'</label>');
	    			}
