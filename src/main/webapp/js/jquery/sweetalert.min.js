...
var handleButton = function handleButton(event, params, modal) {
  ...
  } else if (targetedCancel && params.cancelButtonColor) {
	normalColor = params.cancelButtonColor;
  }
  ...
  function shouldSetConfirmButtonColor(color) {
    if ((targetedConfirm && params.confirmButtonColor)
    	|| (targetedCancel && params.cancelButtonColor)) {
...
var openModal = function openModal(callback) {
  ...
  var $modal = getModal();
  var $htl=$($modal).find("p").eq(0);
  $($modal).find("p").eq(0).html($htl.text());//修改支持html
...
window.alert = function(text, status, timeout, callback, allowOutsideClick) {
	allowOutsideClick = typeof allowOutsideClick === 'boolean' ? allowOutsideClick : true;
	setTimeout(function() {
		if (status === 'success') {
			swal({title: "Good job!", text: text, type: "success", confirmButtonText: "确定", allowOutsideClick: allowOutsideClick, animation: false}, function() {
				if (typeof callback == 'function') {
					callback();
				}
			});
		} else if (status === 'fail') {
			swal({title: "臣妾做不到啊!", text: text, type: "error", confirmButtonText: "好吧", allowOutsideClick: allowOutsideClick, animation: false}, function() {
				if (typeof callback == 'function') {
					callback();
				}
			});
		} else {
			swal({title: "", text: text, confirmButtonText: "确定", allowOutsideClick: allowOutsideClick, animation: false}, function() {
				if (typeof callback == 'function') {
					callback();
				}
			});
...
window.confirm = function(title, text, success, fail, allowOutsideClick) {
	allowOutsideClick = typeof allowOutsideClick === 'boolean' ? allowOutsideClick : true;
	setTimeout(function() {
		...
		swal({
			...
			function(isconfirm) {
				if (isconfirm && typeof success == 'function') {
					success();
				}
				if (!isconfirm && typeof fail == 'function') {
					fail();
				}
			});
	}, 0);
